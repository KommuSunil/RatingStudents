...
public class SubjectRating {
	...
	public void getSubjectRating(String subject, Distributions distributions, Assignments a) {
		...
		for(int i=0;i<a.assignments.size();i++) {
			if(a.assignments.get(i).get(2).toString().equalsIgnoreCase(subject)) {
				found = true;
			}
		}
		if(found == true) {
			for(Map.Entry<String, Integer> entry : a.students.entrySet()) {		
				float test = 0, tc =0, qs = 0, qc=0, lab=0, labc=0, prj=0, prjc=0;			
				for(int i=0;i<a.assignments.size();i++) {
					if(a.assignments.get(i).get(2).toString().equalsIgnoreCase(subject)) {

						if(a.assignments.get(i).get(1).toString().equalsIgnoreCase(entry.getKey())) {
			...
			System.out.println(Arrays.asList("Student Name", "Test Score", "Quiz Score", "Lab Score","Project Score", "Overall Rating(%)"));
